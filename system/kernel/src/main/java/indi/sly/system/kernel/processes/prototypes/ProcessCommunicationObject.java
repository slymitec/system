...
public class ProcessCommunicationObject extends ABytesValueProcessPrototype<ProcessCommunicationDefinition> {
    ...
    public UUID createPort(Set<UUID> sourceProcessIDs) {
        ...
        try {
            ...
            permission.setId(processToken.getAccountID());
            permission.setType(UserType.ACCOUNT);
            permission.setScope(AccessControlScopeType.THIS);
            permission.setValue(PermissionType.FULLCONTROL_ALLOW);
            accessControls.add(permission);
            permission = new AccessControlDefinition();
            permission.setId(this.factoryManager.getKernelSpace().getConfiguration().SECURITY_GROUP_USERS_ID);
            permission.setType(UserType.GROUP);
            permission.setScope(AccessControlScopeType.THIS);
            permission.setValue(PermissionType.CREATECHILD_WRITEDATA_ALLOW);
            accessControls.add(permission);
            securityDescriptor.setPermissions(accessControls);
